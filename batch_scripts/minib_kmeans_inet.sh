...
#SBATCH --job-name=minib_inet
#SBATCH --output=outfiles/minib_inet.out.%j
#SBATCH --error=outfiles/minib_inet.out.%j
...
for backbone in ${BACKBONES[@]}; do
    srun bash -c "CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python minib_kmeans.py --backbone $BACKBONES_DIR/$backbone --dataset $DATASET --num-clusters 1000;"
done
